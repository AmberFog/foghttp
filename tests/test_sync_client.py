...
def test_pending_request_queue_full(sync_http_server: str) -> None:
...
def test_zero_pending_queue_allows_available_connection(sync_http_server: str) -> None:
...
def test_pending_requests_are_tracked_while_waiting(sync_http_server: str) -> None:
    limits = foghttp.Limits(max_active_requests=0, max_pending_requests=1)
    timeouts = foghttp.Timeouts(pool=0.2)

    with foghttp.Client(limits=limits, timeouts=timeouts) as client:
        with ThreadPoolExecutor(max_workers=1) as executor:
            future = executor.submit(client.get, sync_http_server)
            deadline = time.monotonic() + 1.0
            while time.monotonic() < deadline and client.stats().pending_requests == 0:
                time.sleep(0.005)
