...
def test_sync_read_timeout_applies_to_stalled_response_body(
    sync_http_server: str,
    body_path: str,
) -> None:
    ...
    with foghttp.Client(timeouts=timeouts) as client:
        with pytest.raises(foghttp.ReadTimeout, match="response body read timeout expired") as exc_info:
            client.get(f"{sync_http_server}{body_path}")
...
def test_sync_invalid_delayed_eof_size_falls_through_without_crashing_server(
    sync_http_server: str,
    size_segment: str,
) -> None:
    url = f"{sync_http_server}{DELAYED_EOF_UNKNOWN_SIZE_BYTES_PATH}/{size_segment}"
...
def test_sync_aggregate_buffered_response_budget_limits_concurrent_bodies(
    sync_http_server: str,
) -> None:
    ...
    with foghttp.Client(limits=limits) as client:
        with ThreadPoolExecutor(max_workers=CONCURRENT_RESPONSE_COUNT) as executor:
            futures = [executor.submit(client.get, url) for _ in range(CONCURRENT_RESPONSE_COUNT)]
...
def test_sync_aggregate_buffered_response_budget_releases_between_sequential_bodies(
    sync_http_server: str,
) -> None:
    limits = foghttp.Limits(
        max_response_body_size=BODY_LIMIT,
    ...
    url = f"{sync_http_server}/bytes/{BODY_LIMIT}"
