...
keep-alive reuse. Timeout, cancellation, transport error, memory budget
rejection, body-size rejection, and decoding failure increment
`response_body_aborted`. These counters describe Rust-side buffered body
outcomes; they do not claim exact physical socket reuse.
