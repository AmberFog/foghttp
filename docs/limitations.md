...
| socket lifecycle telemetry granularity | `TransportStats` and `dump_transport_state()["origins"]` expose opened, open-failed, closed, reused, aborted, idle-timeout eviction, active, and idle tracked connection counters for the current HTTP/1 path; these are not a complete log of internal reuse attempts. Current integration hooks expose neither every attempt/recovery outcome nor authoritative close reasons, so dedicated failed-reuse and close-reason counters are not available. See [lifecycle semantics](lifecycle.md#operational-state). |
| proxy endpoint diagnostics | `dump_proxy_diagnostics()` separately exposes physical HTTP proxy connection and HTTPS CONNECT outcomes for at most the configured HTTP/HTTPS endpoints; it is diagnostic rather than an alert/export contract and does not support SOCKS, PAC, or TLS-to-proxy endpoints |
...
| diagnostic snapshot transactionality | `stats()`, `dump_transport_state()`, `dump_pool_diagnostics()`, and `dump_proxy_diagnostics()` include `schema_version` and a monotonic `snapshot_sequence`, but the `dump_*` APIs remain diagnostic snapshots rather than lock-protected SLA transactions; use `stats()` for alert-oriented low-cardinality metrics |
