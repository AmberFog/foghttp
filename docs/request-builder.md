...
| proxy / `trust_env` | Partly supported | Plain HTTP proxy routing is supported through client-level `proxy=` or `trust_env=True`; HTTPS `CONNECT` is planned separately. |
